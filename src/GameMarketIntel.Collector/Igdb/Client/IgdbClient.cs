...
public sealed class IgdbClient(HttpClient httpClient, IOptions<IgdbPocOptions> options) : IIgdbClient
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var countResponse =
            await response.Content.ReadFromJsonAsync<IgdbCountResponse>(
                cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (gameIds.Count > 500)
        {
            throw new ArgumentOutOfRangeException(
                nameof(gameIds),
                "An IGDB games query cannot request more than 500 IDs.");
        }
        ...
        if (gameIds.Any(gameId => gameId <= 0))
        {
            throw new ArgumentOutOfRangeException(
                nameof(gameIds),
                "Game IDs must be positive.");
        }

        if (gameIds.Distinct().Count() != gameIds.Count)
        {
            throw new ArgumentException(
                "Game IDs must be distinct.",
                nameof(gameIds));
        }
